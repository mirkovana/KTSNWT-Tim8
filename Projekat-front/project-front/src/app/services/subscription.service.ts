...
export class SubscriptionService {
  ...
  unsubscribe(id: number) {
    this.http.delete<any>('http://localhost:8080/api/offers/unsubscribe/' + id, { headers: new HttpHeaders().append('Authorization', `Bearer ${this.auth_token}`) }
    ).subscribe(data => {
      console.log(data);
    }
    );
  ...
  subscribe(id: number) {
    ...
    let header = new HttpHeaders().append('Authorization', `Bearer ${this.auth_token}`);
